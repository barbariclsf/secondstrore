...
import com.squirrel.util.GoodsRecommender;
...
public class GoodsServiceImpl implements GoodsService {
    ...
    @Autowired
    private GoodsRecommender goodsRecommender;
    ...
    public List<Goods> seletOrderAndGoods(){
       return goodsMapper.seletOrderAndGoods();
    }

    @Override
    public List<Goods> recommendGoodsBasedItem(int userID, int size) {
        List<Long> recommendedItems=null;
        try {
            recommendedItems=  goodsRecommender.itemBasedRecommend(new Integer(userID).longValue(),size);
        } catch (TasteException e) {
            e.printStackTrace();
        }
        List<Goods> goodsList = goodsMapper.queryGoodsByIds(recommendedItems);
        return goodsList;
    }
